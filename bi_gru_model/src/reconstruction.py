...
INPUT_FOLDER = str(Path(__file__).parent.parent / "data" / "processed")

initialize_scalers_called = False

SCALER_PATH = str(Path(__file__).parent.parent / "scalers")
...
def main():
    ...
    model.load_state_dict(torch.load(str(Path(__file__).parent.parent / "models" / "BiGRU.pth"), map_location=device))
    model.eval()
    
    df_before = pd.read_parquet(os.path.join(INPUT_FOLDER, day, flight_name, "adsb_before.parquet"))
    df_ads_c = pd.read_parquet(os.path.join(INPUT_FOLDER, day, flight_name, "adsc.parquet"))
    df_after = pd.read_parquet(os.path.join(INPUT_FOLDER, day, flight_name, "adsb_after.parquet"))
    # Gap 1: before → ads_c
    ...
    output_folder = str(Path(__file__).parent.parent / "data" / "reconstructions")
    os.makedirs(output_folder, exist_ok=True)
    cur_ouptput_folder = os.path.join(output_folder, day, flight_name)
    os.makedirs(cur_ouptput_folder, exist_ok=True)
    ads_b_ads_c.to_parquet(os.path.join(cur_ouptput_folder, "adsb_before-adsc.parquet"))
    ads_c_ads_b.to_parquet(os.path.join(cur_ouptput_folder, "adsc-adsb_after.parquet"))
    ads_b_ads_b.to_parquet(os.path.join(cur_ouptput_folder, "adsb_before-adsb_after.parquet"))
    full.to_parquet(os.path.join(cur_ouptput_folder, "full_reconstruction.parquet"))
