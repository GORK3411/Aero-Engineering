...
def main():
    ...
    reconstructed_path = str(Path(__file__).parent.parent / "data" / "reconstructions")
    cleaned_data_folder = str(Path(__file__).parent.parent / "data" / "processed")

    df_ads_b_before = pd.read_parquet(os.path.join(cleaned_data_folder, step, selected_flight, "adsb_before.parquet"))
    ...
    plot_combined_map(
    {
        "Reconstruction": (final_flight,          "orange"),
